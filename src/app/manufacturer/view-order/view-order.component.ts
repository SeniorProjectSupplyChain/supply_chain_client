...
export class ViewOrderComponent implements OnInit {
    ...
    this.orderService.getAllOrder().subscribe(
      response => {
        ...
        this.orders = data.data
          .sort(
            (a: any, b: any) => {
              if (a.status.toLowerCase() === 'shipped' && b.status.toLowerCase() !== 'shipped') {
                return -1; // a trước b
              }
              if (a.status.toLowerCase() !== 'shipped' && b.status.toLowerCase() === 'shipped') {
                return 1; // b trước a
            ...
            (a: any, b: any) => {
              if (a.status.toLowerCase() === 'shipping' && b.status.toLowerCase() !== 'shipping') {
                return -1; // a trước b
              }
              if (a.status.toLowerCase() !== 'shipping' && b.status.toLowerCase() === 'shipping') {
                return 1; // b trước a
            ...
            (a: any, b: any) => {
              if (a.status.toLowerCase() === 'approved' && b.status.toLowerCase() !== 'approved') {
                return -1; // a trước b
              }
              if (a.status.toLowerCase() !== 'approved' && b.status.toLowerCase() === 'approved') {
                return 1; // b trước a
  ...
  loadOrderData() {
    ...
    switch (this.selectedOrderStatus) {
      case 'pending':
        this.displayedOrderColumns = ['Index', 'qrCode', 'retailer', 'phoneNumber', 'address', 'status', 'createDate', 'action'];
        this.isPendingStatus = true;
        this.isApproved = false;
        this.isShipping = false;
        this.isShipped = false;
        this.isRejected = false;
        showOrders = this.orders.filter((i: any) => i.status.toLowerCase() == 'pending')
          .sort(
            (a: any, b: any) => {
              if (a.status.toLowerCase() === b.status.toLowerCase()) {
                const timeA = new Date(a.createDate).getTime();
        ...
        this.dataSourceOreder = new MatTableDataSource(showOrders);
        this.dataSourceOreder.paginator = this.orderPaginator;
        break;
      case 'approved':
        this.displayedOrderColumns = ['Index', 'qrCode', 'retailer', 'phoneNumber', 'address', 'status', 'updateDate', 'action'];
        this.isPendingStatus = false;
        this.isApproved = true;
        this.isShipping = false;
        this.isShipped = false;
        this.isRejected = false;
        showOrders = this.orders.filter((i: any) => i.status.toLowerCase() == 'approved')
          .sort(
            (a: any, b: any) => {
              if (a.status.toLowerCase() === b.status.toLowerCase()) {
                const timeA = new Date(a.updateDate).getTime();
      ...
      case 'shipping':
        ...
        this.isShipped = false;
        this.isRejected = false;
        showOrders = this.orders.filter((i: any) => i.status.toLowerCase() == 'shipping')
          .sort(
            (a: any, b: any) => {
              if (a.status.toLowerCase() === b.status.toLowerCase()) {
                const timeA = new Date(a.updateDate).getTime();
      ...
      case 'shipped':
        ...
        this.isShipped = true;
        this.isRejected = false;
        showOrders = this.orders.filter((i: any) => i.status.toLowerCase() == 'shipped')
          .sort(
            (a: any, b: any) => {
              if (a.status.toLowerCase() === b.status.toLowerCase()) {
                const timeA = new Date(a.updateDate).getTime();
      ...
      case 'rejected':
        ...
        showOrders = this.orders.filter((i: any) => i.status.toLowerCase() == 'rejected')
          .sort(
            (a: any, b: any) => {
              if (a.status.toLowerCase() === b.status.toLowerCase()) {
                const timeA = new Date(a.updateDate).getTime();
  ...
  applyOrderFilter(event: Event) {
    ...
    this.dataSourceOreder.filter = filterValue.trim().toLowerCase();
  }
}
